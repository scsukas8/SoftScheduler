...
export default function TaskCard({ task, onComplete, onEdit }) {
  const { isOverdue, daysRemaining, hoursRemaining, hoursTotal } = calculateTimeRemaining(task.completed_at, task.interval_days);
  const { isWiggle, formattedTime, isCritical } = formatTimeRemaining(isOverdue, daysRemaining, hoursRemaining, task.wiggle_room, hoursTotal);

  // Fade out distant tasks (starting at 7 days, maxing at 14 days)
  let opacity = 1;
  if (daysRemaining > 7) {
    const fadeAmount = (daysRemaining - 7) / 7;
    opacity = Math.max(0.4, 1 - (fadeAmount * 0.6));
  }

  return (
    <div 
      className={`task-card ${isCritical ? 'critical' : isWiggle ? 'wiggle' : ''}`}
      style={{ '--task-color': task.color, opacity }}
    >
      <div className="task-indicator" style={{ backgroundColor: isCritical ? '#ff6b6b' : isWiggle ? '#ffd93d' : task.color }} />
